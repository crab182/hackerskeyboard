...
1. **Embed** the query into BGE-M3 dense + sparse vectors (`hybrid.rs`,
   `embed_query`). Default backend is `gpu-runtime` `/embed`; swap to in-process
   `ort`/`fastembed` behind the same signature (spec §16).
...
3. **Rerank** (`rerank.rs`): a `bge-reranker-v2-m3` cross-encoder scores each
   ...
   backends behind `RerankBackend`: in-process ONNX (`ort`, default) or
   `gpu-runtime` `/rerank`.
...
and the Qdrant `VectorStore` calls are in place; deferred logic bodies are marked
`// TODO:` (embedding, ONNX/HTTP rerank, condense prompt, chunk-text hydration).
